...
Leds::Leds(LedConfig_t *config, Logger *logger) :
        ...
        _logger(logger),
        _config(config),
        // nope: NEO_RGBW, NEO_WRGB
        _inner_ring(config->inner_ring_num_leds, config->inner_ring_pin, NEO_GRB + NEO_KHZ800, pxl_buffer),
        _outer_ring(config->outer_ring_num_leds, config->outer_ring_pin, NEO_GRB + NEO_KHZ800, pxl_buffer),
    ...
    // shift --> 256*4 -> 10
    const uint16_t shift_min = 256*4*4;
    _outer_ring_min_multiplier = static_cast<uint16_t>((shift_min * static_cast<uint32_t>(_config->outer_ring_num_leds)) / 60) + 1;
    _inner_ring_min_multiplier = static_cast<uint16_t>((shift_min * static_cast<uint32_t>(_config->inner_ring_num_leds)) / 60);
...
void Leds::setTimeOnOuterRing(uint8_t hour, uint8_t min, uint16_t ms_in_min) {
    // hours
    uint16_t center_pxl = static_cast<uint16_t>((static_cast<uint32_t>(hour) * 5 * _outer_ring_min_multiplier) >> 12);
    uint16_t center_pxl_m1 = (center_pxl == 0) ? _config->outer_ring_num_leds -1 : center_pxl - 1;
    uint16_t center_pxl_m2 = (center_pxl_m1 == 0) ? _config->outer_ring_num_leds -1 : center_pxl_m1 - 1;
    ...
    // minutes
    _outer_ring.setPixelColor(static_cast<uint16_t>((static_cast<uint32_t>(min) * _outer_ring_min_multiplier) >> 12), _dimmed_color.minutes);
...
void Leds::setTimeOnInnerRing(uint8_t hour, uint8_t min, uint16_t ms_in_min) {
    // hours
    uint16_t center_pxl = static_cast<uint16_t>((static_cast<uint32_t>(hour) * 5 * _inner_ring_min_multiplier) >> 12);
    ...
    // minutes
    _inner_ring.setPixelColor(static_cast<uint16_t>((static_cast<uint32_t>(min) * _inner_ring_min_multiplier) >> 12), _dimmed_color.minutes);
...
void Leds::update(const Time & current_time, const uint16_t & milliseconds_in_minute) {
    if ((current_time.min == 0) && (current_time.sec == 0)) {
        fullHourSpin(current_time.hr);
    }
    showTime(current_time, milliseconds_in_minute);
}
...
void Leds::showTime(const Time & current_time, uint16_t ms) {
